...
if (spawn_timer <= 0) {
    ...
    var random_index = irandom(5); // Adjust the range based on the number of objects in the array
    var selected_object = tileObjects[random_index];

    // Create the selected object at the spawner's position
    var spawned_object = instance_create_layer(x, y, "true_back", selected_object);

    // Set a speed for the object to move upwards
     selected_object.vspeed = -.5 // Adjust the speed as needed
